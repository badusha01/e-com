    ...
    class ProductInfo extends HTMLElement {
      ...
      setQuantityBoundries() {
        ...
        if (max) {
          ...
          // Disable input or button if cart quantity reaches or exceeds max
          if (data.cartQuantity >= max) {
            this.quantityInput.disabled = true; // Disable quantity input
            const addButton = document.querySelector(".add-to-cart-button"); // Update selector as needed
            if (addButton) addButton.disabled = true; // Disable add button
          } else {
            this.quantityInput.disabled = false; // Enable quantity input if under the limit
            const addButton = document.querySelector(".add-to-cart-button"); // Update selector as needed
        ...
        // Ensure input value respects the new `min`
        this.quantityInput.value = min;
